...
const sf::Color QuadMap::s_defaultColor = sf::Color(0, 0, 0, 0);

static sf::Vector2f Mul(const sf::Vector2f& a, const sf::Vector2f& b) {
...
sf::Vector2f QuadMap::GetSize() const {
    return sf::Vector2f(m_size.x * m_quadSize.x, m_size.y * m_quadSize.y); // zamijenjeno cwiseMul
}

void QuadMap::CreateQuad(uint32_t x, uint32_t y, uint32_t index) {
    sf::Vector2f translation = Mul(sf::Vector2f(x, y), m_quadSize);
    ...
    */
    ...
    m_Vertices[offset + 1].position = sf::Vector2f(m_quadSize.x, 0.0f) + translation;
    m_Vertices[offset + 2].position = sf::Vector2f(m_quadSize.x, m_quadSize.y) + translation;
    m_Vertices[offset + 3].position = sf::Vector2f(m_quadSize.x, m_quadSize.y) + translation;
    m_Vertices[offset + 4].position = sf::Vector2f(0.0f, m_quadSize.y) + translation;
...
void QuadMap::CreateGrid() {
    ...
    for (uint32_t i = 0; i < size.y / m_quadSize.y; i++) {
        ...
        v0.color = sf::Color(150, 150, 150, 200);
        v1.color = sf::Color(150, 150, 150, 200);
